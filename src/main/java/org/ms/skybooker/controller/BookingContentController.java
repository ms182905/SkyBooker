...
public class BookingContentController {
    ...
    @FXML
    private SearchableComboBox<String> passengerComboBox;
    @FXML
    private SearchableComboBox<String> flightComboBox;
    @FXML
    ...
    public void initialize() {
        ...
        Task<Pair<Pair<List<Booking>, List<Passenger>>, List<Flight>>> combinedTask = new Task<>() {
            @Override
        ...
        combinedTask.setOnSucceeded(event -> {
            Pair<Pair<List<Booking>, List<Passenger>>, List<Flight>> result = combinedTask.getValue();
            ...
            var passengerComboBoxOptions = passengers.stream()
                    .map(passenger -> passenger.getName().getValue() + " " + passenger.getSurname().getValue() + ": " + passenger.getPhoneNumber().getValue())
                    .toList();
            passengerComboBox.getItems().addAll(passengerComboBoxOptions);

            var flightComboBoxOptions = flights.stream()
                    .map(flight -> flight.getFlightNumber().getValue() + " " + flight.getStartPoint().getValue() + " - " + flight.getDestination().getValue() + " - " + flight.getDepartureDateTime().getValue())
                    .toList();
            flightComboBox.getItems().addAll(flightComboBoxOptions);
        });

        Thread combinedThread = new Thread(combinedTask);
        combinedThread.setDaemon(true);
        ...
        validator.createCheck()
                .withMethod(c -> {
                    ...
                    if (selectedPassenger == null || selectedPassenger.isEmpty()) {
                        ...
                        if (tableView.getItems().stream().anyMatch(e -> e.getFlightNumber().getValue().equals(flightNumber) && e.getPhoneNumber().getValue().equals(phoneNumber))) {
    ...
    private Booking getBooking() {
        ...
        return new Booking(passengerId, passenger.getName().get() + " " + passenger.getSurname().getValue(), passenger.getPhoneNumber().getValue(), flightNumber,flight.getStartPoint().getValue() + " - " + flight.getDestination().getValue(), flight.getDepartureDateTime().getValue());
    }
        ...
        if (selectedBooking != null) {
            ...
            alert.showAndWait().ifPresent(response -> {
                if (response == buttonTypeOK) {
                    ...
                    DatabaseManager.deleteBooking(selectedBooking.getFlightNumber().getValue(), selectedBooking.getPassengerId());
                    clearSelection();
                }
            });
        }
